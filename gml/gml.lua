...
local function drawLabel(label)
  if not label:isHidden() then
    ...
    label.renderTarget.set(screenX,screenY, unicode.sub(label.text, 1,label.width) .. (" "):rep(label.width-unicode.len(label.text)))
    label.visible=true
  end
end
...
local function drawButton(button)
  if not button:isHidden() then
    ...
    if unicode.len(text)>bodyW then
      text=unicode.sub(text, 1,bodyW)
    else
      textX=bodyX+math.floor((bodyW-unicode.len(text))/2)
    end
...
local function drawTextField(tf)
  if not tf:isHidden() then
    ...
    visibleText=visibleText..(" "):rep(tf.width-unicode.len(visibleText))
...
local function insertTextTF(tf,text)
  ...
  tf.cursorIndex=tf.cursorIndex+unicode.len(text)
...
local function addTextField(gui,x,y,width,text)
  ...
  tf.onClick=function(tf,tx,ty,button)
      tf.selectEnd=0
      tf.cursorIndex=math.min(tx+tf.scrollIndex-1,unicode.len(tf.text)+1)
      tf:draw()
    end
  ...
  tf.onBeginDrag=function(tf,tx,ty,button)
      ...
      if button==0 then
        tf.selectOrigin=math.min(tx+tf.scrollIndex,unicode.len(tf.text)+1)
  ...
  tf.onDrag=function(tf,tx,ty)
      if tf.dragging then
        ...
        tf.cursorIndex=math.max(math.min(dragX+tf.scrollIndex-1,unicode.len(tf.text)+1),1)
        ...
        if dragX<1 or dragX>tf.width then
          ...
          tf.dragTimer=event.timer(dragRate,function()
              assert(tf.gui.running)
                tf.cursorIndex=math.max(math.min(tf.cursorIndex+dragStep,unicode.len(tf.text)+1),1)
  ...
  tf.keyHandler=function(tfclear,char,code)
      ...
      elseif code==keyboard.keys.right then
        local prevCI=tf.cursorIndex
        if tf.cursorIndex<unicode.len(tf.text)+1 then
          tf.cursorIndex=tf.cursorIndex+1
      ...
      elseif code==keyboard.keys["end"] then
        local prevCI=tf.cursorIndex
        if tf.cursorIndex~=unicode.len(tf.text)+1 then
          tf.cursorIndex=unicode.len(tf.text)+1
      ...
      elseif code==keyboard.keys.delete then
        if tf.selectEnd~=0 then
          tf:removeSelected()
          dirty=true
        elseif tf.cursorIndex<=unicode.len(tf.text) then
  ...
  tf.gotFocus=function()
    ...
    if unicode.len(effText)>tf.width then
      tf.scrollIndex=unicode.len(effText)-tf.width+3
    else
      tf.scrollIndex=1
    end
    tf.cursorIndex=unicode.len(effText)+1
    tf:draw()
  end
...
local function scrollFolder(sb)
  local folder=sb.folder
  
  local modShift = (sb.scrollPos-1) % folder.cellStepY
  local startRow = math.floor(sb.scrollPos / folder.cellStepY)
  for j=1,#folder.grid do
    for i=1, #folder.grid[j] do
      local n = (startRow+j-1)*folder.maxColumns + i
      local cell = folder.grid[j][i]
      cell.posY = -modShift + (j-1)*folder.cellStepY
      cell.data = #folder.items>=n and folder.items[n] or nil
      
      cell.state = nil
      if(folder.selected) then
        for _,v in pairs(folder.selected) do
          if n==v then cell.state = "selected" end
        end
      end
...
-- Table with icons, like Windows folders
local function addFolder(gui,x,y, width, height, items, spaceX, spaceY, iconPattern)
  
  -- Standart values
  spaceX = spaceX or 1
  spaceY = spaceY or 0
  iconPattern = iconPattern or {"■── □ ──┐","│       │","│       │","└─     ─┘"}

  -- Create base component. Copy parameters 
  local folder = compositeBase(gui,x,y,width,height,"folder",true)
  folder.items  = items  -- All items in folder for view
  folder.spaceX = spaceX -- Horisontal space between cells
  folder.spaceY = spaceY --   Vertical space between cells

  -- Compute icon pattern size
  local iconW = 3
  for _,v in pairs(iconPattern) do
    local lnLen = unicode.len(v)
    if lnLen > iconW then iconW = lnLen end
  end
  local iconH = #iconPattern
  folder.iconW = iconW
  folder.iconH = iconH
  folder.cellStepX = iconW + spaceX
  folder.cellStepY = iconH + spaceY
  
  local maxColumns   = math.floor( (folder.bodyW-1)/(iconW + spaceX) )
  local maxRows      = math.floor( (folder.bodyH-1)/(iconH + spaceY) )
  folder.maxColumns  = maxColumns -- Count of columns in folder box
  folder.maxRows     = maxRows    -- Count of rows. folder.grid have 1 more row for scrolling

--  folder.scrollBar.posY=1
--  folder.scrollBar.height=folder.height
--  folder.scrollBar.length=folder.height
  
  folder.scrollBar = addScrollBarV(folder,folder.bodyW,folder.bodyY,folder.bodyH,1,scrollFolder)
  folder.scrollBar.class="folder"
  folder.scrollBar.folder=folder

  folder.grid={}
  for _y=1, maxRows+1 do
    table.insert(folder.grid, {})
    for _x=1, maxColumns do
      local icoX = folder.bodyX + (iconW + spaceX)*(_x-1)
      local icoY = folder.bodyY + (iconH + spaceY)*(_y-1)
      local cell  = compositeBase(folder,icoX,icoY,folder.iconW,folder.iconH,"folder",true)      
      
      cell.onClick = function(obj,tx,ty,button)
        if obj.state then obj.state = nil else obj.state = "selected" end
        obj:draw()
      end
      
      cell.draw = function(obj)
        if not obj:isHidden() then
          local screenX,screenY=obj:getScreenPosition()
          local fg, bg, sfg, sbg=findStyleProperties(obj, "text-color","text-background","selected-color","selected-background")
          if obj.state == "selected" then
            obj.renderTarget.setForeground(sfg)
            obj.renderTarget.setBackground(sbg)
          else
            obj.renderTarget.setForeground(fg)
            obj.renderTarget.setBackground(bg)
          end
          if obj.data then
            for i=1,#iconPattern do
              obj.renderTarget.set(screenX,screenY+i-1, iconPattern[i])
            end
            
            for i=1,#obj.data do
              local ln = obj.data[i]
              local lnLen = unicode.len(ln)
              obj.renderTarget.set(screenX + math.floor((iconW-lnLen)/2),screenY+i-1, ln)
            end
          else
            obj.renderTarget.set(screenX + math.floor(iconW/2), screenY + math.ceil(iconH/2), "▪")
          end
        end
      end
      
      folder:addComponent(cell)
      table.insert(folder.grid[_y], cell)
    end
  end

  -- Change content of folder
  folder.updateItems = function(fl, newItems)
    fl.items=newItems
    
    local itemsCount   = newItems and #newItems or 0 
    local columns      = math.min (itemsCount, fl.maxColumns)
    local rows         = math.ceil(itemsCount/ fl.maxColumns)
    local contentWidth = fl.iconW*columns + fl.spaceX*(columns-1)
    local contentHeight= fl.iconH*rows    + fl.spaceY*(rows   -1)
    
    local excessLines =  contentHeight - fl.bodyH
    fl.scrollBar.scrollPos = 1
    fl.scrollBar.scrollMax = math.max(1, excessLines + 1)
    updateScrollBarGrip(fl.scrollBar)
    scrollFolder(fl.scrollBar)
  end
  if items then folder:updateItems(items) end
  
  folder.onClick = function(folder,tx,ty,button)
    if tx==folder.width then
      folder.scrollBar:_onClick(ty,button)
    else      
      for _y=1, #folder.grid do
        for _x=1, #folder.grid[_y] do
          local cell = folder.grid[_y][_x]
          if cell:contains(tx,ty) then
            cell:onClick(tx-cell.posX+1,ty-cell.posY+1,button)
          end
        end
      end
    end
  ...
  folder.draw=function(fl)
    if not fl:isHidden() then
      local styles=getAppliedStyles(fl)
      drawBorder(fl,styles)
      fl.scrollBar:draw()
      for j=1,#fl.grid do
        for i=1, #fl.grid[j] do
          fl.grid[j][i]:draw()
        end
      end
    end
