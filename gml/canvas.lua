...
local colorutils=require("gml.colorutils")
...
function canvasMeta.set(canvas,x,y,string)
  ...
  canvas.buffer=unicode.sub(canvas.buffer, 1,index-1)..span..unicode.sub(canvas.buffer, index+#span)
end

local function canvas_get8(canvas,x,y)
  local index=canvas_posToIndex8(canvas,x,y)
  local pixel=unicode.sub(canvas.buffer, index,index+4)
...
local function canvas_get4(canvas,x,y)
  ...
  local pixel=unicode.sub(canvas.buffer, index,index+2)
...
function canvasMeta.copy(canvas,x,y,w,h,dx,dy)
  ...
  for y=sy,ey,ystep do
    ...
    local str=unicode.sub(canvas.buffer, si,ei)
    print("row "..y.." segment len : "..unicode.len(str))
    si,ei=canvas.posToIndex(sx+dx,y+dy),canvas.posToIndex(ex+dx,y+dy)+pixelw-1
    canvas.buffer=unicode.sub(canvas.buffer, 1,si-1)..str..unicode.sub(canvas.buffer, ei+1)
  end
...
function canvasMeta.fill(canvas,x,y,w,h,char)
  ...
  local xoff=(x-1)*(unicode.len(canvas.colorStr)+1)+1
  local yoffstep=canvas.width*(unicode.len(canvas.colorStr)+1)
  local yoff=(y-1)*yoffstep
  for y=y, y+h-1 do
    canvas.buffer=unicode.sub(canvas.buffer, 1,xoff+yoff-1)..line..unicode.sub(canvas.buffer, xoff+yoff+lineLen)
    yoff=yoff+yoffstep
  end
end
